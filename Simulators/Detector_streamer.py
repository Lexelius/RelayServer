...
sample = 3 ######## Pick your sample here!
...
while True:
	...
	data_array = np.ndarray(shape=imshape, dtype=data.dtype, buffer=data[i, :, :])## buffer=np.linspace(i, npx-1+i, npx, dtype="uint32")) ## buffer=np.random.rand(imshape[0],imshape[1]))
